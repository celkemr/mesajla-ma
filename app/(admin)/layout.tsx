    ...
    <div className="flex min-h-screen bg-[#f0f2f7]">
      ...
      <aside className="hidden lg:flex w-60 flex-col shrink-0" style={{ background: 'linear-gradient(180deg, #1a1f36 0%, #111827 100%)' }}>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-150 group relative ${
                  active ? 'text-white' : 'text-[#9ca3af] hover:text-white hover:bg-white/5'
                }`}
              ...
              >
                ...
                <span className={active ? 'text-indigo-400' : 'text-[#6b7280] group-hover:text-[#9ca3af]'}>{item.icon}</span>
        ...
        <div className="px-3 py-4 border-t border-white/5 space-y-3">
          ...
          <Link
            href="/profile"
            className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-[13px] transition-all duration-150 ${pathname === '/profile' ? 'text-white bg-white/10' : 'text-[#6b7280] hover:text-white hover:bg-white/5'}`}
          >
      ...
      <main className="flex-1 overflow-auto flex flex-col min-w-0">
        ...
        {/* Mobil üst bar - sadece logo */}
        <div className="lg:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-slate-200/80 shrink-0">
          ...
          <Link
            href="/profile"
            className={`p-2 rounded-lg transition-colors ${pathname === '/profile' ? 'text-indigo-600 bg-indigo-50' : 'text-slate-400 hover:text-slate-700 hover:bg-slate-100 active:bg-slate-200'}`}
            title="Hesabım"
          >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
              <path fillRule="evenodd" d="M18.685 19.097A9.723 9.723 0 0 0 21.75 12c0-5.385-4.365-9.75-9.75-9.75S2.25 6.615 2.25 12a9.723 9.723 0 0 0 3.065 7.097A9.716 9.716 0 0 0 12 21.75a9.716 9.716 0 0 0 6.685-2.653Zm-12.54-1.285A7.486 7.486 0 0 1 12 15a7.486 7.486 0 0 1 5.855 2.812A8.224 8.224 0 0 1 12 20.25a8.224 8.224 0 0 1-5.855-2.438ZM15.75 9a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" clipRule="evenodd" />
        ...
        {/* Sayfa içeriği - altta bottom nav kadar padding */}
        ...
        {navItems.filter(item => item.href !== '/users' || me?.isSuperAdmin).map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              onClick={() => { if (item.badge) setNewCount(0); }}
              className="flex-1 flex flex-col items-center justify-center py-2 gap-0.5 relative transition-colors active:bg-slate-50"
            >
              {/* Aktif üst çizgi */}
              {active && (
                <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-b-full" style={{ background: 'linear-gradient(90deg,#6366f1,#8b5cf6)' }} />
              )}
              <span className={`relative transition-colors ${active ? 'text-indigo-600' : 'text-slate-400'}`}>
                {item.icon}
                {showBadge && (
                  <span className="absolute -top-1 -right-1.5 text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center text-white" style={{ background: 'linear-gradient(135deg,#ef4444,#f97316)' }}>
